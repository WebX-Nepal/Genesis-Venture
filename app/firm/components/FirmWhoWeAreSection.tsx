  ...
  return (
    <section className="w-full bg-white py-14 sm:py-16">
      <div className="layout-7xl">
        <h2 className="text-center font-montserrat text-[clamp(1.7rem,3.2vw,2.4rem)] font-semibold leading-[1.1] text-[#162e54]">
          Who We Are
        </h2>
        <div className="mt-8 grid w-full grid-cols-1 gap-4 sm:mt-10 md:grid-cols-2">
          {items.map((item) => (
            <article key={item.title} className="border border-[#162e54]/12 bg-white p-6 sm:p-7">
              ...
              <p className="mt-3 text-[0.98rem] font-montserrat leading-relaxed text-[#162e54]/80">{item.text}</p>
            </article>
          ))}
        </div>
      </div>
